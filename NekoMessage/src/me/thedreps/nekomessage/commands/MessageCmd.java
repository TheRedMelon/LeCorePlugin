...
public class MessageCmd extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		ProxiedPlayer reciver = proxy.getPlayer(args[0]);
		
		if (reciver == null){ //Checks to see if player is online
		...
		player.sendMessage(new ComponentBuilder(ChatColor.LIGHT_PURPLE + "[" + ChatColor.RESET + "You " + ChatColor.LIGHT_PURPLE + "-> "
				+ rank.getColoredRank(reciver.getUniqueId()) + ChatColor.RESET + " " + reciver.getDisplayName()
				+ ChatColor.LIGHT_PURPLE + "] " + ChatColor.RESET
				+ msg).create());
		
		reciver.sendMessage(new ComponentBuilder(ChatColor.LIGHT_PURPLE + "["
